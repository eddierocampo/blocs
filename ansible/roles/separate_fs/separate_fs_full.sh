...
lvcreate -n lvtmp --size 10G vgroot
lvcreate -n lvvar --size 5G vgroot
lvcreate -n lvhome --size 10G vgroot
lvcreate -n lvvartmp --size 5G vgroot
lvcreate -n lvvarlog --size 5G vgroot
lvcreate -n lvvarlogaudit --size 10G vgroot
...
mkdir /tmp1 /var1 /home1 /vartmp /varlog /varlogaudit /scripts

echo "Montando las particiones temporales"
mount /dev/vgroot/lvtmp /tmp1
...
echo '/dev/vgroot/lvtmp           /tmp                xfs     nodev,nosuid          0 0' >> /etc/fstab
echo '/dev/vgroot/lvhome          /home               xfs     nodev                 0 0' >> /etc/fstab
echo '/dev/vgroot/lvvar           /var                xfs     defaults              0 0' >> /etc/fstab
echo '/dev/vgroot/lvvartmp        /var/tmp            xfs     nodev,nosuid          0 0' >> /etc/fstab
